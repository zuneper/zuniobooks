...
async function startServer() {
  ...
  app.post('/api/auth/register', async (req: Request, res: Response) => {
    try {
      ...
      if (dbService.findUserByUsername(username)) {
  ...
  app.get('/api/books', (req: AuthRequest, res: Response) => {
    ...
    if (q) {
      books = books.filter((b) => {
        ...
        const inNarrator = b.narrator.toLowerCase().includes(q);
        ...
        const inEpisodes = episodes.some((ep) => ep.title.toLowerCase().includes(q));
        return inTitle || inAuthor || inNarrator || inDesc || inGenre || inEpisodes;
      });
    }
  ...
  app.put('/api/books/:id', requireAdmin, upload.single('cover'), (req: AuthRequest, res: Response) => {
    try {
      ...
      if (req.file) {
        ...
        updates.coverUrl = customCoverUrl;
      }
  ...
  app.post('/api/books/:bookId/episodes', requireAdmin, upload.single('audio'), (req: AuthRequest, res: Response) => {
    try {
      ...
      if (!audioUrl) {
        return res.status(400).json({ error: 'Audio file or audio URL is required' });
      }
      ...
      const newEpisode = dbService.createEpisode({
        ...
        bookId,
        title: title || `Chapter ${nextTrackNumber}`,
        audioUrl,
  ...
  app.put('/api/episodes/:id', requireAdmin, (req: AuthRequest, res: Response) => {
    ...
    const updates: any = {};
    if (title) updates.title = title;
    if (trackNumber) updates.trackNumber = parseInt(trackNumber, 10);
    if (durationSeconds) updates.durationSeconds = parseFloat(durationSeconds);

    const updated = dbService.updateEpisode(req.params.id, updates);
    ...
    const books = dbService
      ...
      .map((b) => {
        ...
        return {
          ...b,
          episodesCount: episodes.length,
          totalDurationSeconds: episodes.reduce((acc, e) => acc + e.durationSeconds, 0),
          isFavorite: true,
        };
      });
    res.json(books);
  ...
  app.post('/api/progress', requireAuth, (req: AuthRequest, res: Response) => {
    ...
    if (!episodeId || !bookId) {
      return res.status(400).json({ error: 'Episode ID and Book ID required' });
    }
    const prog = dbService.saveProgress(
      req.user.id,
      episodeId,
      bookId,
      positionSeconds || 0,
  ...
  app.get('/api/progress', requireAuth, (req: AuthRequest, res: Response) => {
    if (!req.user) return res.status(401).json({ error: 'Auth required' });
    const progressList = dbService.getUserProgress(req.user.id);
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`🚀 Zuniobooks server active on http://0.0.0.0:${PORT}`);
  });
}
